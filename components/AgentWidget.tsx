    ...
    if (n8nWebhookUrl) {
      ...
      try {
        ...
        if (Array.isArray(parsedData) && parsedData.length > 0) {
           ...
           agentReply = firstItem.reply || firstItem.output || firstItem.message || firstItem.response || firstItem.text || JSON.stringify(firstItem);
        } else if (typeof parsedData === 'object' && parsedData !== null) {
           let obj = parsedData;
           if (obj.json) obj = obj.json; // Desempacota se vier no formato raw do n8n
           
           agentReply = obj.reply || obj.output || obj.message || obj.response || obj.text || JSON.stringify(obj);
        ...
        if (typeof agentReply === 'string' && agentReply.trim().startsWith('{') && agentReply.trim().endsWith('}')) {
          try {
